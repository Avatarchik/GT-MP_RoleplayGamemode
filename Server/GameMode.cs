...
namespace Roleplay
{
    ...
    {
        private static List<RoleplayScript> _loadedScripts = new List<RoleplayScript>();
        ...
        private void API_onResourceStart()
        {
            ...
            {
                if (item.IsSubclassOf(typeof(RoleplayScript)) && !item.IsAbstract)
                {
                    logger.Info($"Loading class {item.Name}");
                    var o = Activator.CreateInstance(item) as RoleplayScript;
                    if (o != null)
            ...
            API.addWhitelistURL("https://gtaassets.tam.moe/");
        ...
        public void CommandReload(Client player)
        {
            ...
            DbContext.ItemInformations.LoadAsync();
        }
        ...
        private void RegisterScriptCommands(RoleplayScript script)
        {
        ...
        private static void ShutDown(bool kickPlayers, bool stopServer)
        {
            try
            {
                ...
                if (kickPlayers)
                {
                    ...
                    {
                        // Maybe Save Players?
                        if (player.IsLoggedIn() && player.Account().IsSpawned)
                        {
                            if (player.Account().CurrentCharacter != null)
                            {
                                player.Account().CurrentCharacter.Save(player);
                            }
                        }
